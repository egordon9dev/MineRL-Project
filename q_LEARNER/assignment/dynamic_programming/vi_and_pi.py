...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while delta >= tol:
        ...
        for idx_s in range(nS):
            action = np.zeros((nA,))
            val_prev = value_function[idx_s]
            for idx_a in range(nA):
                Q_val_cur = 0

                if len(P[idx_s][idx_a]) == 1:
                    p, nextstate, reward, term = P[idx_s][idx_a][0]
                
                    if term:
                        long_term = 0
                    else:
                        long_term = value_function[nextstate]

                    Q_val_cur = (reward + gamma*long_term)*p
                else: 
                    for i in range(len(P[idx_s][idx_a])):
                        p, nextstate, reward, term = P[idx_s][idx_a][i]
                ...
                action[idx_a] = Q_val_cur
            
            value_function[idx_s] = np.max(action)
            policy[idx_s] = np.argmax(action)

            if delta < np.abs(val_prev - value_function[idx_s]):
                delta = np.abs(val_prev - value_function[idx_s])
